...
def test_correct_results_influenza():
    ...
    return_data = response.json()["return_data"]
    print(return_data)

    assert response.status_code == 200
    assert "J10.1" in return_data["cid"]
    assert "tosse" in return_data["result"]
    assert "dor de garganta" in return_data["result"]
    assert "vírus" in return_data["result"]
    assert "identificado" in return_data["result"]
